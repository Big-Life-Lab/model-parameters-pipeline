---
title: "Introduction to Model Parameters Pipeline"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Introduction to Model Parameters Pipeline}
...
The Model Parameters Pipeline is an R package for applying transformations to data according to the [Model Parameters](https://github.com/Big-Life-Lab/model-parameters) specification developed by Big Life Lab. This package implements a pipeline for sequential data transformations that are commonly used in predictive health models.
...
4. **RCS**: Applies restricted cubic spline transformations for non-linear relationships
...
Let's walk through a simple example. The easiest way to use the package is with a model export file that specifies all configuration files:
...
Instead of a model export file, you can specify files directly:
...
You can pass data frames instead of file paths:
...
If you need to apply the same model to multiple datasets (e.g., processing batches), reuse the model object for better performance:

```{r eval=FALSE}
# First run - model files are loaded and cached
mod <- run_model_pipeline(
  root_dir = "path/to/model/directory",
  data = batch1_data,
  model_export = "path/to/model-export.csv"
...
# Subsequent runs - reuse the cached model configuration
mod <- run_model_pipeline(
  root_dir = "path/to/model/directory",
  data = batch2_data,
  model_export = "path/to/model-export.csv",
...
The pipeline returns a model object with the transformed data in the `$df` component:
...
2. **Center**: Continuous variables are centered (e.g., `age_centered = age - 50`)
3. **Dummy**: Categorical variables become binary indicators (e.g., `sex_male = 1` if male, 0 otherwise)
4. **Interaction**: Combinations of variables are multiplied (e.g., `age_sex = age_centered * sex_male`)
...
- For detailed information about the Model Parameters specification, see the [Model Parameters Reference Documentation](https://big-life-lab.github.io/model-parameters/5-reference.html)
- To add new transformation steps, see the `ADDING_NEW_STEP.md` guide in the package
- To report issues or request features, visit the [issue tracker](https://github.com/Big-Life-Lab/model-parameters-pipeline/issues)

## Additional Resources

- [Big Life Lab GitHub](https://github.com/Big-Life-Lab)
- [Model Parameters Specification](https://github.com/Big-Life-Lab/model-parameters)
- [HTNPoRT Model](https://github.com/Big-Life-Lab/htnport)
- [Predictive Algorithms Repository](https://github.com/Big-Life-Lab/predictive-algorithms)
